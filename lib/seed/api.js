import { discoverFromOpenAPI, discoverFromTRPC } from "../discover/index.js";
...
/**
 * Seed entities via tRPC mutations.
 *
 * @param {object} plan - Parsed plan object
 * @param {Array} entities - Discovered entities array (must have toolName)
 * @param {object} [options]
 * @param {object} options.faker - Faker instance
 * @param {string} [options.auth] - Authorization header value
 * @param {boolean} [options.dryRun] - Skip HTTP calls
 ...
 * @returns {Promise<{seeded: object, errors: Array, dryRun?: boolean, records?: object}>}
 */
export async function seedViaTRPC(plan, entities, options = {}) {
  const { faker, auth, dryRun = false, fetch: fetchFn = fetch } = options;
    ...
    for (let i = 0; i < planEntity.count; i++) {
      ...
      if (!dryRun) {
        try {
          ...
          const url = `${plan.baseUrl}/trpc/${procedureName}`;
          const headers = { "Content-Type": "application/json" };
          if (auth) headers["Authorization"] = auth;
...
/**
 * High-level orchestrator: read plan, discover entities, seed via API or tRPC.
 *
 * @param {string} planSource - YAML string or file path
 * @param {object} options
 * @param {string} [options.openapi] - OpenAPI spec URL or file path
 * @param {string} [options.trpc] - tools.json file path (tRPC mode)
 * @param {object} options.faker - Faker instance
...
export async function executeSeed(planSource, options = {}) {
  const { openapi, trpc, ...seedOptions } = options;

  if (openapi && trpc) {
    throw new Error("Specify either --openapi or --trpc, not both");
  }
  if (!openapi && !trpc) {
    throw new Error("Either openapi or trpc option is required to discover entity schemas");
  }

  const plan = await readPlan(planSource);

  if (trpc) {
    const discovery = await discoverFromTRPC(trpc);
    return seedViaTRPC(plan, discovery.entities, seedOptions);
  }
